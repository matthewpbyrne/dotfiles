...
elif [ ! -d "$ANTIDOTE_DIR/.git" ] || [ ! -f "$ANTIDOTE_DIR/antidote.zsh" ]; then
	printf '%s\n' "Error: '$ANTIDOTE_DIR' exists but does not appear to be a valid antidote clone." >&2
	printf '%s\n' "Please remove or fix this directory and re-run this installer." >&2
	exit 1
fi
